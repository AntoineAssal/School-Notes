...
- So now each process may need up to 4x1million bytes = 4MB of physical address space.
- Why is this bad?
...
### Multi-level Paging
- Use a two-level paging algorithm, in which basicallty the page table itself is also paged. So we will have an outer page table and an inner page table.
...
- The output that we get from looking up P1 in the outer page table will be combined with P2 gives us the input to look up in the inner page table.
